// Runs the wordle solver -- either specify a word for it to search
// towards, like this:
// $ ./solver <secret word>
//
...
//   assume that result points at enough memory for a string of length
//   5. 6 bytes long
...
// per line.
// Also, this function sets the value pointed at by *num_words to be the number of words read
// null-terminated.
...
int main(int argc, char **argv) {
  ...
  do {
    ...
    if (!success) {
      // If it didn't get it right, filter down the vocabulary!
      for (int i = 0; i < 5; i++) {
        if (result[i] == 'x') {
          // only remove words containing this letter if it doesn't occur
          // twice, but it only occurs once(weird edge case)
          // marks letters as gray if the guess is twice but they occur once.
          char letter = guess[i];
